...
class PaymentScreen extends ConsumerWidget {
  ...
  Widget _buildPaymentMethodForm(PaymentResponse paymentData) {
    return PaymentMethodFormField(
      payments: paymentData.paymentMethods,
      onchange: (newValue) {
        debugPrint("Selected Payment : $newValue");
        selectedPaymentMethod = int.parse(newValue!.paymentId);
      },
      onSaved: (value) {
  ...
  Widget _buildNotesField() {
    return NotesFieldWidget(onChange: (value) {
      notes = value;
    });
  }

  Widget _buildSubmitButton(BuildContext context, WidgetRef ref, Cart cart) {
      ...
      return CustomButtonWidget(
        ...
        onTap: () {
          if (_formKey.currentState!.validate()) {
            ...
            _confirmPayment(context, ref, cart);
          }
        },
        isFiled: true,
        height: 50,
  ...
  void _confirmPayment(BuildContext context, WidgetRef ref, Cart cart) {
    debugPrint("Tap ");
    debugPrint("Your Notes are: $notes");
